...
def get_modules_dir():
    ...
    return os.path.normpath(script_dir + "/../docs/gcn-modules/")
...
def parse_guide(modules_dir, guide_path):
    ...
    with open(guide_path, 'r') as file:
        ...
        match = re.search('<a href="/gcn/docs/gcn-modules/([\w-]+/archives/[\w-]+-java-maven.zip)">', guide)
        if match is None:
            raise Exception("Could not find maven zip in guide: " + guide_path)
        result["maven_zip"] = modules_dir + "/" + match.group(1)

        match = re.search('<a href="/gcn/docs/gcn-modules/([\w-]+/archives/[\w-]+-java-gradle.zip)">', guide)
        if match is None:
